...
## ADR-0021: 회원 탈퇴는 202 접수 즉시 로컬 세션을 종료하고 재인증을 별도 분기한다

- 배경(문제)
  - 서버 회원 탈퇴는 계정을 즉시 `DELETION_PENDING`으로 차단한 뒤 외부 provider 해제와
    DB·Redis 삭제를 백그라운드에서 수행하는 비동기 작업이다.
  - 보안 상 최근 10분 이내 전체 로그인이 필요하며 refresh token 갱신은 재인증이 아니다.
  - Google·Naver는 서버가 provider 해제용 token을 보관하지 않아 클라이언트 SDK 연결 해제가
    서버 요청보다 먼저 실행되어야 한다.
- 선택지(대안)
  - A. 탈퇴 상태 API를 `COMPLETED`까지 폴링한 뒤 로그아웃한다.
  - B. `202 Accepted`를 최종 클라이언트 성공으로 보고 즉시 로컬 세션을 종료한다.
    `REAUTHENTICATION_REQUIRED`만 현재 상태를 유지해 다시 로그인을 요구한다.
- 결정(무엇을 선택)
  - B를 선택한다.
  - 설정 화면은 복구 불가 영향을 최종 확인한 뒤 boolean `confirmation: true`를
    `DELETE /auth/account`로 전송한다.
  - 탈퇴 요청은 글로벌 Dio의 만료 임박·`401` refresh 자동 재시도 대상에서 제외한다.
  - Google 계정은 `google_id`가 있을 때 `disconnect()`, Naver는 현재 SDK access token이 있을 때
    `logOutAndDeleteToken()`을 호출한 뒤 서버 탈퇴를 요청한다.
  - `202`, `401`, `ACCOUNT_DELETION_IN_PROGRESS`에서는 로컬 JWT·소셜 세션·계정 캐시를
    정리하고 로그인 화면으로 이동한다.
  - `REAUTHENTICATION_REQUIRED`는 설정 화면과 JWT를 유지하고 전용 안내를 표시한다.
    사용자가 다시 로그인을 선택하면 세션을 종료하고, 전체 로그인 후 탈퇴를 다시 확인한다.
- 근거(왜)
  - `202` 시점부터 일반 API 사용이 차단되므로 삭제 완료를 기다리는 화면은 유효한
    앱 상태를 제공하지 못한다.
  - 상태 폴링 중 사용자 DB 레코드가 삭제되면 정상적으로 `401`이 발생할 수 있어
    클라이언트 완료 판정에 추가 가치가 없다.
  - 탈퇴 요청의 `401`을 refresh로 바꾸면 최근 로그인 요구를 우회하지는 못하면서
    서버 오류 의미와 클라이언트 처리를 불필요하게 바꾼다.
- 결과/영향(좋은 점/트레이드오프)
  - 탈퇴 접수 후 추가 일반 API 호출이 발생하지 않고 계정 단위 Riverpod 캐시도 즉시
    무효화된다.
  - 재인증을 token refresh와 구분하지만, 현재 라우팅에서는 재로그인 후 사용자가
    설정으로 돌아와 탈퇴를 다시 확인해야 한다.
  - Google/Naver 연결 해제가 실패하면 서버 탈퇴 요청을 보내지 않아 provider 연결이
    남은 채 계정 데이터만 삭제되는 부분 성공을 막는다.
- 추후 과제(언제 다시 평가)
  - 로그인 provider를 명시적으로 보존하는 재인증 route가 추가되면 설정 화면에서 로그인을
    완료한 뒤 탈퇴 확인으로 복귀하는 흐름을 검토한다.
