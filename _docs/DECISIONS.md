...
- 결과/영향(좋은 점/트레이드오프)
  ...
  - 선택일은 흰색 surface 배경과 2px primary dark outline, 굵은 날짜 텍스트로 식별한다.
  - 오늘은 날짜 의미 색상을 유지하고 기존 primary 밑줄로 구분한다.
  - 친구 캘린더는 현재 공휴일 데이터를 로드하지 않으므로 이 결정에서는 일요일·토요일 의미 색상만 적용한다.
- 추후 과제(언제 다시 평가)
  - 실제 기기에서 2px outline의 시각적 무게를 확인하고, 친구 캘린더에도 한국 공휴일 데이터가 필요해질 때 공휴일 로딩 범위를 별도로 결정한다.
